    ...
    if quality_score >= 90:
        ...
        insights.append(
            Insight(
                ...
                description=(
                    "La qualité reste au-dessus du seuil de 80%, critère de succès atteint."
                ),
                severity="success",
                metric="Qualité",
                value=f"{quality_score}%",
        ...
        if abs(memory_growth) < 0.1:
            insights.append(
                Insight(
                    ...
                    description=(
                        "Le coût contextuel reste quasi-plat sur la durée de la conversation."
                    ),
                    severity="success",
                    metric="Croissance",
    ...
    if trap_ratio >= 0.8:
        ...
        insights.append(
            Insight(
                ...
                description=(
                    "La plupart des faits cachés ne sont pas retrouvés. Vérifier embeddings."
                ),
                severity="critical",
                metric="Pièges",
                value=f"{passed}/{total}",
